...
import vn.edu.hust.airplanecontext.domain.exception.NotAcceptableException;
import vn.edu.hust.airplanecontext.domain.factory.FactoryGenerator;
import vn.edu.hust.airplanecontext.domain.message.command.airplane.AddSeatToAirplaneCommand;
import vn.edu.hust.airplanecontext.domain.message.command.airplane.AssignAirplaneToFlightCommand;
import vn.edu.hust.airplanecontext.domain.message.command.airplane.MakeAReservationCommand;
import vn.edu.hust.airplanecontext.domain.message.command.airplane.RegisterNewAirplaneCommand;
...
import vn.edu.hust.airplanecontext.domain.message.event.airplane.SeatAddedToAirplaneEvent;
...
public class Airplane {
    ...
    public void handle(MakeAReservationCommand command) {
        if (!this.state.equals(AirplaneState.SCHEDULED) ||
            !this.flight.getFlightId().equals(command.flightId())
        ) {
            throw new NotAcceptableException(
                    "Airplane" + this.airplaneAggregateId + " is not available for reserving"
            );
        }
        if (this.numberEmptySeat == 0) {
            throw new NotAcceptableException("Airplane" + this.airplaneAggregateId + " has been full");
        }
//        resolver.detectConflicts( _  -> this.numberEmptySeat == 0);
        AggregateLifecycle.apply(new NumberEmptySeatDecreasedEvent(
                command.airplaneId(),
                command.seatId(),
                this.numberEmptySeat - 1
        ), MetaData.from(Map.of(
                "flight.id", command.flightId(),
